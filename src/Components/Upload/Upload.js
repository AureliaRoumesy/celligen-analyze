  ...
  useEffect(() => {
    setHeaderToken(() => {
      axios.get(`${conf.url}/manipinprogress/`)
        ...
        .catch((error) => {
          message.error('Problème lors de la récupération des manips en cours.', 3);
        });
    });
  }, []);
    ...
    if (manipSelected === 'new') {
      setHeaderToken(() => {
        axios.post(`${conf.url}/upload`, files)
          .then(res => {
            if (res.status === 200) {
              message.success('Données bien sauvegardées', 3);
            } else if (res.status === 400) {
              message.error("Nom de manip déjà utilisé, merci d'en choisir un autre.", 3);
            }
            setLoading(false);
          })
          .catch(() => {
            message.error('Problème de serveur, manip non sauvegardée. Merci de réessayer', 3);
            setLoading(false);
          });
      });
    } else {
