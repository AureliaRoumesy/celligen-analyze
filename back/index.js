...
const privateKEY = fs.readFileSync('./back/.private.key', 'utf8');
const publicKEY = fs.readFileSync('./back/.public.key', 'utf8');
...
app.get('/manipinprogress', verifyToken, (req, res) => {
  jwt.verify(req.token, publicKEY, verifyOptions, (err, authData) => {
    if (err) {
      ...
      celligenModel.find({ finish: false }, (err, manip) => {
        if (err) {
          console.log(err);
        } else {
          res.json(manip);
  ...
  jwt.verify(req.token, publicKEY, verifyOptions, (err, authData) => {
    if (err) {
      ...
      bcrypt.hash(values.passwordSignUp, 10, (err, hash) => {
        const newUser = new userModel({ name: values.nameSignUp, password: hash, email: values.emailSignUp });
        newUser.save((err, auth) => {
          if (err) {
            console.log(err);
...
app.post('/login/', (req, res) => {
  const values = req.body;
  console.log('login');
  userModel.findOne({ email: values.emailSignIn }, (err, result) => {
    if (err) {
      console.log(err);
